...
% Doris comments:
% - a lot of the apostrophes in "user's" did not show up on the pdf. Not sure if its the way you're compiling on your end, but I could get it working on my side.
% - you should use \ref{<name_of_label>} for your Figure labels so that the pdf will cross link the figures
...
\label{fig:figure1}
\end{figure}
Everyday, we interact with the world through our skin. The human skin senses important events that happen closest to us, and serves as an expressive medium when adorned with tattoo art. In this paper, we present Skintillates, a class of temporary tattoo epidermal wearable interactive devices. %skintillates/skintillate plural/singular? 
Skintillate devices presented in this paper include electronic tattoo as passive and active on-skin display, capacitive sensors for mobile devices electronic instrument control and strain gauge for posture detection. Similar to traditional tattoo%s
, Skintillates can be customized to be a variety of different sizes and colors to fit the user's intended functions. Moreover, we demonstrate an accessible fabrication method that involves %all (awk)
all commercially-available materials and easy-to-obtain equipment. 
Skintillates is inspired by a line of research in micron-thin epidermal electronics pioneered by material scientists.%you're not going to cite Rogers here?
 Since these epidermal electronics directly contacts the skin, they can be made into extremely accurate, yet comfortable, sensors. However, due to their intricate fabrication method, epidermal sensors remain to be a device mainly used in specialized medical and military applications. %However, due to their complex fabrication method, most epidermal sensors are reserved for specialized medical and military applications. 
However, there is a clear need in the field of human-computer interactions for on-skin wearable electronics to enable a natural and always-available interactions with the electronics and data around us [cites]. %electronic devices and public space?
In addition to giving commands using a wearable device, Skintillates can also serve as a programmable/addressable LED temporary tattoo display. %this seems to downplay the possible applications of skintillate as pertaining solely to these two categories. Maybe listing: sensors, LED display, interacting with other wearable devices.
...
% In this study, we develop a fabrication process without the need of cleanroom equipment or the extreme temperatures, but at the same time enable users to customize the device both electronically and aesthetically. %[Also you never really talk about extreme temperatures anywhere, I'm guessing this is for the liquid metals?]
...
\begin{enumerate}
  ...
  \item The electronic has to be conductive enough to support basic functions. %[basic functions? or sensitive /sensor measurements?]
...
%[can you remove the orangey reddish hue on the pictures in Fig 8 ?]
\begin{figure}[!h]
\centering
...
\label{fig:figure8}
\end{figure}
Using the human body as a conductor to form a closed circuit to turn on a light is common science experiment, and this sensing method %can 
also enable interesting interactions - such as turning bananas into switches - made popular by MakeyMakey.  %serve as unusual medium of interactions such as MakeyMakey's ``bannana switches"
